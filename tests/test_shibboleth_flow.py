...
def test_full_login_flow_drives_real_credential_form_and_synthetic_bounce():
    ...
    responses.add(responses.GET, f"{BASE}/shib_login.php", body=idp_login_html, status=200)
    responses.add(
        responses.POST,
        f"{BASE}/idp/profile/SAML2/Redirect/SSO",
        body=(
            '<html><body><form method="post" action="/Shibboleth.sso/SAML2/POST">'
    ...
    # synthetic SAML-relay hop was auto-submitted, and is_logged_in() then
    # found no login form on the (mocked) personal desktop.
    submitted_credentials = responses.calls[1].request.body
    assert "j_username=xy1234" in submitted_credentials
    assert "j_password=secret" in submitted_credentials
